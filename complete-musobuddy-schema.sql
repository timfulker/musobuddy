-- Complete MusoBuddy Database Schema - All 37 Tables
-- Snake_case columns for PostgreSQL compatibility
-- Run this in both Development and Production Supabase projects

-- Enable necessary extensions
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

-- 1. Session storage table (required for Express sessions)
...
-- 2. Users table (main user accounts)
CREATE TABLE users (
...
-- 15. Bookings table (comprehensive)
...
CREATE TABLE event_sync_mapping (
  id SERIAL PRIMARY KEY,
  user_id VARCHAR NOT NULL REFERENCES users(id) ON DELETE CASCADE,
...
CREATE TABLE contract_extraction_patterns (
  id SERIAL PRIMARY KEY,
...
CREATE INDEX idx_security_user_service ON security_monitoring(user_id, api_service);
CREATE INDEX idx_security_created_at ON security_monitoring(created_at);
CREATE INDEX idx_security_suspicious ON security_monitoring(suspicious);
CREATE INDEX idx_client_communications_user ON client_communications(user_id);
CREATE INDEX idx_client_communications_client_email ON client_communications(client_email);
CREATE INDEX idx_client_communications_sent_at ON client_communications(sent_at);
