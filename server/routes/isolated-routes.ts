...
export function registerIsolatedRoutes(app: Express) {
  ...
  app.post('/api/debug/contracts/send-email', async (req: any, res) => {
    try {
      ...
      const result = await emailService.sendContractEmail(contract, userSettings, subject, signingPageUrl, customMessage || '[TEST EMAIL]', contract.userId);
  ...
  async function handleIsolatedContractEmail(req: any, res: any) {
    try {
      ...
      try {
        await emailService.sendContractEmail(updatedContract, userSettings, subject, signingPageUrl, customMessage || '', contract.userId);
