...
import { apiUsageStats, apiUsageTracking, users } from '../../shared/schema';
...
app.post('/api/admin/send-beta-email', authenticate, async (req: AuthenticatedRequest, res) => {
  ...
  try {
    ...
    if (emailTemplate) {
      // Format message for HTML and text
      const htmlMessage = message ? `<p class="lead" style="background:#f0f9ff; padding:12px; border-radius:6px; border-left:4px solid #6C5CE7;">${message}</p>` : '';
            ...
            <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" class="container" style="border-radius:16px; overflow:hidden; box-shadow:0 6px 24px rgba(17,24,39,0.12);">
              ...
              <tr>
                <td class="section" style="font-family:Inter,Segoe UI,Roboto,Helvetica,Arial,sans-serif;">
                  ...
                  <p class="lead">Hi <strong>${firstName}</strong>,</p>

                  ${htmlMessage}

                  <p class="lead">Thanks for jumping in to help us shape MusoBuddy. Your feedback will directly influence what we build next.</p>
...
// Update existing template to new format (message at top)
app.post('/api/admin/beta-email-templates/update-to-new-format', authenticate, async (req: AuthenticatedRequest, res) => {
  try {
    // Check admin permissions
    if (!req.user?.isAdmin) {
      return res.status(403).json({ error: 'Admin access required' });
    ...
    console.log('🔄 [ADMIN] Updating existing template to new format');

    // Get the active template
    const existingTemplate = await storage.getActiveBetaEmailTemplate();
    if (!existingTemplate) {
      return res.status(404).json({
        error: 'No active template found to update'
      });
    }

    // Update HTML to move message to top
    const updatedHtmlBody = existingTemplate.htmlBody
      .replace(
        /<p class="lead">Hi <strong>\{\{firstName\}\}<\/strong>, thanks for jumping in to help us shape MusoBuddy\./,
        '<p class="lead">Hi <strong>{{firstName}}</strong>,</p>\n\n                  {{message}}\n\n                  <p class="lead">Thanks for jumping in to help us shape MusoBuddy.'
      )
      .replace(
        /\{\{message\}\}\s*<p class="muted"/,
        '<p class="muted"'
      );

    // Update text to move message to top
    const updatedTextBody = existingTemplate.textBody
      .replace(
        /Hi \{\{firstName\}\},\s*Thanks for jumping/,
        'Hi {{firstName}},\n\n{{message}}\nThanks for jumping'
      )
      .replace(
        /\{\{message\}\}\s*Need help\?/,
    ...
    await storage.updateBetaEmailTemplate(existingTemplate.id, {
      htmlBody: updatedHtmlBody,
      textBody: updatedTextBody
    });

    console.log(`✅ [ADMIN] Template updated to new format`);

    res.json({
      success: true,
      message: 'Template updated successfully - personal message now appears at the top'
    });
...
// Initialize default beta email template
app.post('/api/admin/beta-email-templates/init-default', authenticate, async (req: AuthenticatedRequest, res) => {
  try {
    // Check admin permissions
    if (!req.user?.isAdmin) {
      return res.status(403).json({ error: 'Admin access required' });
    ...
    console.log('🎨 [ADMIN] Initializing default beta email template');

    // Check if an active template already exists
    const existingTemplate = await storage.getActiveBetaEmailTemplate();
    if (existingTemplate) {
      return res.status(409).json({
        error: 'An active beta email template already exists',
        template: existingTemplate
      });
    }

    // Create the default template with the hardcoded content
...
<html lang="en" xmlns:v="urn:schemas-microsoft-com:vml" xmlns:o="urn:schemas-microsoft-com:office:office">
  ...
  <body style="margin:0; padding:0; background:#0b0b10;">
    ...
    const defaultTextTemplate = `Welcome to the MusoBuddy Beta!
...
https://www.musobuddy.com`;
    ...
    const newTemplate = await storage.createBetaEmailTemplate({
      name: 'Default Beta Invitation',
      description: 'Default template for beta program invitations with 90-day trial',
      subject: 'Welcome to MusoBuddy Beta - Code: {{customCode}}',
    ...
    console.log(`✅ [ADMIN] Default beta email template created:`, {
      id: newTemplate.id,
      name: newTemplate.name
    });
    ...
    res.json({
      success: true,
      message: 'Default beta email template created successfully',
      template: newTemplate
    });

  } catch (error: any) {
    console.error('❌ [ADMIN] Failed to initialize default template:', error);
    res.status(500).json({
      success: false,
      error: error.message
    });
...
app.get('/api/admin/api-usage-stats', authenticate, async (req: AuthenticatedRequest, res) => {
  try {
    // Check admin permissions
    if (!req.user?.isAdmin) {
      return res.status(403).json({ error: 'Admin access required' });
    ...
    console.log('📊 [ADMIN] Fetching API usage statistics');

    // Get total stats
    const totalStats = await db
      .select({
        totalRequests: sql<number>`COALESCE(SUM(${apiUsageStats.totalRequests}), 0)`,
        totalCost: sql<string>`COALESCE(SUM(${apiUsageStats.totalCost}), 0)`,
        blockedUsers: sql<number>`COUNT(CASE WHEN ${apiUsageStats.isBlocked} = true THEN 1 END)`,
        activeUsers: sql<number>`COUNT(DISTINCT ${apiUsageStats.userId})`,
      })
      .from(apiUsageStats);
    ...
    const recentActivity = await db
      .select({
        ...
        cost: sql<string>`COALESCE(SUM(${apiUsageTracking.estimatedCost}), 0)`,
      })
      .from(apiUsageTracking)
      .where(sql`${apiUsageTracking.timestamp} >= ${oneDayAgo}`);

    res.json({
      success: true,
      stats: {
        totalRequests: Number(totalStats[0]?.totalRequests || 0),
        totalCost: parseFloat(totalStats[0]?.totalCost || '0'),
        blockedUsers: Number(totalStats[0]?.blockedUsers || 0),
        activeUsers: Number(totalStats[0]?.activeUsers || 0),
        last24Hours: {
          requests: Number(recentActivity[0]?.requests || 0),
          cost: parseFloat(recentActivity[0]?.cost || '0'),
        },
      },
    });
  } catch (error: any) {
    console.error('❌ [ADMIN] Failed to fetch API usage stats:', error);
    res.status(500).json({
      success: false,
      error: error.message
    });
...
// Get detailed API usage data per user
app.get('/api/admin/api-usage-data', authenticate, async (req: AuthenticatedRequest, res) => {
  try {
    // Check admin permissions
    if (!req.user?.isAdmin) {
      return res.status(403).json({ error: 'Admin access required' });
    ...
    const { sortBy = 'totalCost', sortOrder = 'desc' } = req.query;

    console.log(`📊 [ADMIN] Fetching API usage data (sort: ${sortBy} ${sortOrder})`);
    ...
    const sortFieldMap: { [key: string]: any } = {
      ...
      totalCost: apiUsageStats.totalCost,
      lastActivity: apiUsageStats.lastActivity,
      riskScore: apiUsageStats.riskScore,
    };

    const sortField = sortFieldMap[sortBy as string] || apiUsageStats.totalCost;

    // Get usage stats with user information
    const usageData = await db
      .select({
        ...
        totalCost: apiUsageStats.totalCost,
        lastActivity: apiUsageStats.lastActivity,
        isBlocked: apiUsageStats.isBlocked,
        blockReason: apiUsageStats.blockReason,
        riskScore: apiUsageStats.riskScore,
        serviceBreakdown: apiUsageStats.serviceBreakdown,
      })
      .from(apiUsageStats)
      .leftJoin(users, eq(apiUsageStats.userId, users.id))
      .orderBy(sortOrder === 'asc' ? asc(sortField) : desc(sortField));

    const responseData = usageData.map(row => ({
      ...row,
      totalCost: parseFloat(row.totalCost?.toString() || '0'),
    }));

    console.log('📊 [ADMIN] Returning API usage data:', {
    ...
    res.json({
      ...
      console.error('❌ [ADMIN] Database query failed:', queryError);
      throw queryError;
    }
  } catch (error: any) {
    console.error('❌ [ADMIN] Failed to fetch API usage data:', error);
    res.status(500).json({
      success: false,
      error: error.message
    });
...
// Get detailed API costs and service status
app.get('/api/admin/api-costs', authenticate, async (req: AuthenticatedRequest, res) => {
  try {
    // Check admin permissions
    if (!req.user?.isAdmin) {
      return res.status(403).json({ error: 'Admin access required' });
    ...
    console.log('💰 [ADMIN] Fetching API cost breakdown');

    // Get usage stats by service
    ...
    usageStats.forEach(stat => {
      ...
      const breakdown = stat.serviceBreakdown as any;
      if (breakdown) {
        if (breakdown.openai) serviceCosts.openai += breakdown.openai.cost || 0;
        if (breakdown.anthropic) serviceCosts.anthropic += breakdown.anthropic.cost || 0;
        if (breakdown.google) serviceCosts.google += breakdown.google.cost || 0;
      }
    });
    ...
    const apiStatus = {
      ...
      twilio: {
        ...
        keyPresent: !!process.env.TWILIO_AUTH_TOKEN,
      },
      what3words: {
    ...
    res.json({
      ...
      data: {
        ...
        usage_estimates: {
          openai: {
            estimated_cost: serviceCosts.openai,
          },
          anthropic: {
            estimated_cost: serviceCosts.anthropic,
          },
          googleMaps: {
            estimated_cost: serviceCosts.google,
          },
        },
    ...
    console.error('❌ [ADMIN] Failed to fetch API costs:', error);
    res.json({
      success: false,
      error: error.message,
      data: {
...
// Block user from API usage
app.post('/api/admin/block-user-api', authenticate, async (req: AuthenticatedRequest, res) => {
  try {
    // Check admin permissions
    if (!req.user?.isAdmin) {
      return res.status(403).json({ error: 'Admin access required' });
    ...
    console.log(`🚫 [ADMIN] Blocking API access for user: ${userId}`);
    ...
    res.json({
      success: true,
      message: 'User API access blocked',
    });
  } catch (error: any) {
    console.error('❌ [ADMIN] Failed to block user API access:', error);
    res.status(500).json({
      success: false,
      error: error.message
    });
...
// Unblock user from API usage
app.post('/api/admin/unblock-user-api', authenticate, async (req: AuthenticatedRequest, res) => {
  try {
    // Check admin permissions
    if (!req.user?.isAdmin) {
      return res.status(403).json({ error: 'Admin access required' });
    ...
    console.log(`✅ [ADMIN] Unblocking API access for user: ${userId}`);
    ...
    res.json({
      success: true,
      message: 'User API access unblocked',
    });
  } catch (error: any) {
    console.error('❌ [ADMIN] Failed to unblock user API access:', error);
    res.status(500).json({
      success: false,
      error: error.message
    });
...
app.post('/api/admin/update-risk-score', authenticate, async (req: AuthenticatedRequest, res) => {
  try {
    // Check admin permissions
    if (!req.user?.isAdmin) {
      return res.status(403).json({ error: 'Admin access required' });
    ...
    if (!userId || riskScore === undefined) {
      return res.status(400).json({ error: 'userId and riskScore are required' });
    }
    ...
    console.log(`⚠️ [ADMIN] Updating risk score for user ${userId}: ${riskScore}`);
    ...
    console.error('❌ [ADMIN] Failed to update risk score:', error);
    res.status(500).json({
      success: false,
      error: error.message
    });
...
// ===== EMAIL DELIVERY ANALYTICS ROUTES =====

// Get email delivery failure analytics
app.get('/api/admin/email-delivery-analytics', authenticate, async (req: AuthenticatedRequest, res) => {
  try {
    // Check admin permissions
    if (!req.user?.isAdmin) {
      return res.status(403).json({ error: 'Admin access required' });
    ...
    console.log(`📊 [ADMIN] Fetching email delivery analytics (last ${days} days)`);

    // Get analytics grouped by provider and failure type
    const analytics = await storage.getEmailFailureAnalytics(parseInt(days as string));

    // Get total failures
    const totalFailures = analytics.reduce((sum, row) => sum + Number(row.count), 0);
    ...
    analytics.forEach(row => {
      ...
      byProvider[provider].totalFailures += Number(row.count);
      byProvider[provider].byType[row.failureType] = Number(row.count);
      byProvider[provider].totalRetries += Number(row.totalRetries || 0);
    });
    ...
    console.error('❌ [ADMIN] Failed to fetch email delivery analytics:', error);
    res.status(500).json({
      success: false,
      error: error.message
    });
...
// Get all email delivery failures with filters
app.get('/api/admin/email-delivery-failures', authenticate, async (req: AuthenticatedRequest, res) => {
  try {
    // Check admin permissions
    if (!req.user?.isAdmin) {
      return res.status(403).json({ error: 'Admin access required' });
    ...
    const { provider, failureType, limit = 100 } = req.query;

    console.log(`📊 [ADMIN] Fetching email delivery failures`, { provider, failureType, limit });

    const filters: any = {};
    if (provider) filters.provider = provider as string;
    if (failureType) filters.failureType = failureType as string;
    if (limit) filters.limit = parseInt(limit as string);

    const failures = await storage.getAllEmailFailures(filters);
    ...
    console.error('❌ [ADMIN] Failed to fetch email delivery failures:', error);
    res.status(500).json({
      success: false,
      error: error.message
    });
...
app.get('/api/admin/email-provider-status', authenticate, async (req: AuthenticatedRequest, res) => {
  try {
    // Check admin permissions
    if (!req.user?.isAdmin) {
      return res.status(403).json({ error: 'Admin access required' });
    ...
    console.log('📧 [ADMIN] Fetching email provider status');

    // Get provider status from email service
    const { emailService } = await import('../core/email-provider-abstraction');
    const providerStatus = emailService.getProvidersStatus();

    // Get recent failure rates (last 24 hours)
    const oneDayAgo = new Date();
    oneDayAgo.setDate(oneDayAgo.getDate() - 1);

    const recentAnalytics = await storage.getEmailFailureAnalytics(1);

    const failureRates: { [key: string]: number } = {};
    recentAnalytics.forEach(row => {
      const provider = row.provider || 'unknown';
      failureRates[provider] = (failureRates[provider] || 0) + Number(row.count);
    });
    ...
    res.json({
      ...
      providers: {
        sendgrid: {
          configured: providerStatus.sendgrid,
          recentFailures: failureRates['sendgrid'] || 0
        },
        mailgun: {
          configured: providerStatus.mailgun,
          recentFailures: failureRates['mailgun'] || 0
        },
        default: providerStatus.defaultProvider
      },
      summary: {
        totalRecentFailures: Object.values(failureRates).reduce((sum, count) => sum + count, 0),
        period: 'Last 24 hours'
      }
    });

  } catch (error: any) {
    console.error('❌ [ADMIN] Failed to fetch email provider status:', error);
    res.status(500).json({
      success: false,
      error: error.message
    });
