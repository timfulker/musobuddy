...
export function registerOnboardingRoutes(app: Express) {
  ...
  app.post('/api/onboarding/complete', authenticate, async (req: AuthenticatedRequest, res) => {
    try {
      ...
      const validation = await routeManager.validateEmailPrefix(onboardingData.emailPrefix);

      if (!validation.valid) {
        console.log('❌ Email prefix validation failed:', validation.error);
        return res.status(400).json({ error: validation.error });
      }

      // Check if email prefix is already taken by another user
      // IMPORTANT: Convert to lowercase for case-insensitive validation
      const existingUser = await storage.getUserByEmailPrefix(onboardingData.emailPrefix.toLowerCase());
      ...
      const updatedUser = await storage.updateUser(userId, {
        ...
        emailPrefix: onboardingData.emailPrefix.toLowerCase(), // CRITICAL for booking emails - normalize to lowercase
