...
export async function registerSettingsRoutes(app: Express) {
  ...
  app.post('/api/email/assign-prefix', authenticate, async (req: AuthenticatedRequest, res) => {
    try {
      ...
      // Check if prefix is already taken
      // IMPORTANT: Normalize to lowercase for case-insensitive validation
      const normalizedPrefix = prefix.toLowerCase();
      const existingUser = await storage.getUserByEmailPrefix(normalizedPrefix);
      if (existingUser) {
        return res.status(400).json({
      ...
      // Update user with email prefix (normalized to lowercase)
      await safeDbCall(() => storage.updateUser(userId, { emailPrefix: normalizedPrefix }), null, 'updateUser');

      const fullEmail = `${normalizedPrefix}@enquiries.musobuddy.com`;
  ...
  app.patch('/api/settings',
    ...
    try {
      ...
      if (processedBody.emailPrefix !== undefined) {
        ...
        // Validate email prefix format (no spaces or special characters allowed)
        if (newPrefix && newPrefix.trim()) {
          ...
          const validation = await routeManager.validateEmailPrefix(newPrefix.trim());

          if (!validation.valid) {
            console.error(`❌ Email prefix validation failed: ${validation.error}`);
            return res.status(400).json({
              error: validation.error,
              field: 'emailPrefix'
            });
        ...
        // IMPORTANT: Check with lowercase to ensure case-insensitive validation
        if (newPrefix && newPrefix.trim()) {
          const existingUser = await safeDbCall(() => storage.getUserByEmailPrefix(newPrefix.trim().toLowerCase()), null, 'getUserByEmailPrefix');
          if (existingUser && existingUser.id !== userId) {
        ...
        // IMPORTANT: Convert to lowercase to ensure case-insensitive matching
        const prefixToSave = newPrefix && newPrefix.trim() ? newPrefix.trim().toLowerCase() : null;
  ...
  app.delete('/api/user/delete-account', authenticate, async (req: AuthenticatedRequest, res) => {
    try {
      ...
      await db.transaction(async (tx) => {
        ...
        // 2. Delete user messages (sent/received)
        await tx.delete(userMessages)
          .where(eq(userMessages.fromUserId, userId));

        // 3. Delete booking-related data (must be before bookings)
        await tx.delete(bookingDocuments)
          .where(eq(bookingDocuments.userId, userId));

        await tx.delete(bookingConflicts)
          .where(eq(bookingConflicts.userId, userId));

        await tx.delete(conflictResolutions)
          .where(eq(conflictResolutions.userId, userId));

        // 4. Delete client communications
        await tx.delete(clientCommunications)
          .where(eq(clientCommunications.userId, userId));

        // 5. Delete invoices (foreign key to bookings)
        await tx.delete(invoices)
          .where(eq(invoices.userId, userId));

        // 6. Delete contracts (foreign key to bookings)
        await tx.delete(contracts)
          .where(eq(contracts.userId, userId));

        // 7. Delete imported contracts
        await tx.delete(importedContracts)
          .where(eq(importedContracts.userId, userId));

        // 8. Delete contract extractions
        await tx.delete(contractExtractions)
        ...
        // 12. Delete compliance documents
        await tx.delete(complianceDocuments)
          .where(eq(complianceDocuments.userId, userId));

        // 13. Delete email templates
        ...
        // 19. Delete unparseable messages
        await tx.delete(unparseableMessages)
          .where(eq(unparseableMessages.userId, userId));

        // 20. Delete user activity logs
        await tx.delete(userActivity)
          .where(eq(userActivity.userId, userId));

        // 21. Delete login history
        await tx.delete(userLoginHistory)
          .where(eq(userLoginHistory.userId, userId));

        // 22. Delete security monitoring
        await tx.delete(securityMonitoring)
          .where(eq(securityMonitoring.userId, userId));

        // 23. Delete user security status
        await tx.delete(userSecurityStatus)
          .where(eq(userSecurityStatus.userId, userId));

        // 24. Delete trial usage tracking
        await tx.delete(trialUsageTracking)
          .where(eq(trialUsageTracking.userId, userId));

        // 25. Delete API usage tracking
        await tx.delete(apiUsageTracking)
          .where(eq(apiUsageTracking.userId, userId));

        // 26. Delete API usage stats
        await tx.delete(apiUsageStats)
          .where(eq(apiUsageStats.userId, userId));

        // 27. Delete user settings
        await tx.delete(userSettings)
          .where(eq(userSettings.userId, userId));

        // 28. Delete user sessions (where sess contains the user info)
        ...
        // 29. Finally, delete the user record
      ...
      try {
        ...
        // Use service role key to delete auth user (admin operation)
        const SUPABASE_URL = process.env.NODE_ENV === 'production'
          ? process.env.SUPABASE_URL_PROD!
          : process.env.SUPABASE_URL_DEV!;

        const SUPABASE_SERVICE_ROLE_KEY = process.env.NODE_ENV === 'production'
          ? process.env.SUPABASE_SERVICE_ROLE_KEY_PROD!
          : process.env.SUPABASE_SERVICE_ROLE_KEY_DEV!;

        if (!SUPABASE_URL || !SUPABASE_SERVICE_ROLE_KEY) {
          throw new Error('Supabase configuration missing - cannot delete auth user');
        }
        ...
        if (deleteAuthError) {
          ...
          throw new Error(`Supabase Auth deletion failed: ${deleteAuthError.message}`);
        } else {
        ...
        // This is a critical failure - user will still exist in auth
      ...
      console.log(`   - Database records: Deleted from 29+ tables`);
      console.log(`   - R2 files: ${deletedFiles} deleted, ${failedFiles} failed`);
      console.log(`   - User email: ${userData.user?.email}`);
      console.log(`   - Supabase Auth: Deleted`);
