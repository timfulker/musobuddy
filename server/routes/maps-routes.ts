...
export function registerMapsRoutes(app: Express) {
  ...
  // Places search endpoint using Places API (New) SearchText method
  app.post('/api/maps/places-search', requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      console.log(`🗺️ Places SearchText (New API): ${query}`);

      // Use Places API (New) SearchText method
      ...
      if (!response.ok) {
        console.log(`❌ Places SearchText failed: ${response.status}`, data);
      ...
      // Transform Places API (New) response to our expected format
