...
export function setupAuthRoutes(app: Express) {
  ...
  app.post('/api/create-checkout-session', async (req, res) => {
    try {
      ...
      // Calculate trial days from user's trialEndsAt field if available
      let trialDays = 30; // Default for regular users
      if (isBetaTester && user?.trialEndsAt && user?.createdAt) {
        // Calculate days between createdAt and trialEndsAt
        const trialEndDate = new Date(user.trialEndsAt);
        const createdDate = new Date(user.createdAt);
        const daysDiff = Math.ceil((trialEndDate.getTime() - createdDate.getTime()) / (1000 * 60 * 60 * 24));
        trialDays = daysDiff > 0 ? daysDiff : 90; // Use calculated days or fallback to 90
        console.log(`🎯 Calculated ${trialDays} trial days from user's trialEndsAt`);
      } else if (isBetaTester) {
        trialDays = 90; // Fallback for beta testers
        console.log('⚠️ No trialEndsAt found, using fallback 90 days for beta tester');
      }
  ...
  app.post('/api/auth/supabase-signup', async (req, res) => {
    try {
      ...
      let isBetaUser = false;
      let betaInvite = null;
      let betaTrialDays = 90; // Default beta trial days
            ...
            if (betaCode && betaCode.status === 'active') {
              ...
              if (!betaCode.expiresAt || new Date() <= new Date(betaCode.expiresAt)) {
                ...
                if (betaCode.currentUses < betaCode.maxUses) {
                  isBetaUser = true;
                  betaTrialDays = betaCode.trialDays || 90; // Use beta code's custom trial days
                  console.log('✅ Valid dynamic beta invite code provided:', effectiveInviteCode);
                  console.log(`🎯 Using ${betaTrialDays} trial days from beta code`);
      ...
      const userData = {
        ...
        trialEndsAt: isBetaUser
          ? new Date(Date.now() + betaTrialDays * 24 * 60 * 60 * 1000) // Use custom trial days from beta code
