...
import { generateInvoiceEmailHTML } from "../core/services";
  ...
  app.post('/api/invoices/send-email', (req: any, res, next) => {
    ...
    try {
      ...
      try {
        ...
        // Generate invoice email HTML using the standalone function
        const emailHtml = generateInvoiceEmailHTML(invoice, userSettings, pdfUrl);
        ...
        // This ensures deliverability since enquiries.musobuddy.com is authenticated in SendGrid
        let fromEmail = 'noreply@enquiries.musobuddy.com'; // Fallback

        try {
          const user = await storage.getUserById(userId);
          if (user?.emailPrefix) {
            fromEmail = `${user.emailPrefix}@enquiries.musobuddy.com`;
            console.log(`📧 [INVOICE] Using personalized from address: ${fromEmail}`);
          }
        } catch (error) {
          console.error('❌ [INVOICE] Failed to fetch user emailPrefix, using fallback:', error);
        }

        // Create reply-to routing address for conversation tracking
        const replyToAddress = invoice.bookingId && userId
          ? `User${userId}-Booking${invoice.bookingId} <user${userId}-booking${invoice.bookingId}@mg.musobuddy.com>`
          : undefined;
