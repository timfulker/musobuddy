...
function calculateDueDateFromPaymentTerms(eventDate: Date, paymentTerms: string): Date {
  const dueDate = new Date(eventDate);

  // Parse payment terms (e.g., "28_days_before", "7_days_after", "on_performance_date")
  if (paymentTerms === 'on_performance_date') {
    return dueDate;
  }

  const match = paymentTerms.match(/(\d+)_days_(before|after)/);
  if (match) {
...
export function registerBookingRoutes(app: Express) {
  ...
  app.post('/api/bookings', 
    ...
    try {
      ...
      // Get user settings for default payment terms
      const userSettings = await storage.getSettings(userId);

      // Calculate payment terms and due date if not provided (use defaults from settings)
      let paymentTerms = req.body.paymentTerms || null;
      let dueDate = req.body.dueDate || null;
      let paymentTermsCustomized = req.body.paymentTermsCustomized || false;

      // If payment terms not provided and we have an event date, calculate from defaults
      if (!paymentTerms && req.body.eventDate && userSettings?.invoicePaymentTerms) {
        paymentTerms = userSettings.invoicePaymentTerms;
        paymentTermsCustomized = false;

        // Calculate due date based on payment terms
        const eventDate = new Date(req.body.eventDate);
        dueDate = calculateDueDateFromPaymentTerms(eventDate, paymentTerms);

        console.log(`💰 Applied default payment terms: ${paymentTerms}, calculated due date: ${dueDate}`);
      }
      ...
      const bookingData = {
        ...
        // Payment terms (source of truth for contracts and invoices)
        paymentTerms,
        dueDate,
  ...
  app.patch('/api/bookings/:id', authenticate, async (req: AuthenticatedRequest, res) => {
    ...
    try {
      ...
      console.log(`🔍 [BOOKING-UPDATE] Booking #${bookingId} update data:`, {
        ...
        paymentTermsCustomized: req.body.paymentTermsCustomized,
