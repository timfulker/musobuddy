...
class EnhancedEmailQueue {
  ...
  async addEmail(requestData: any): Promise<{ jobId: string; queuePosition: number; isDuplicate?: boolean; userId?: string }> {
    ...
    // Step 1: Extract the actual email address from formats like "Name <email@domain>" or just "email@domain"
    let emailAddress = recipientField;
    const angleBracketMatch = recipientField.match(/<([^>]+)>/);
    if (angleBracketMatch) {
      // Found angle brackets, extract email from inside
      emailAddress = angleBracketMatch[1].trim();
    } else {
      // No angle brackets, try to find the email pattern (word@domain)
      const emailMatch = recipientField.match(/([a-zA-Z0-9._-]+@[a-zA-Z0-9.-]+)/);
      if (emailMatch) {
        emailAddress = emailMatch[1];
      }
    }

    // Step 2: Extract prefix from the clean email address
    const emailPrefixMatch = emailAddress.match(/^([a-zA-Z0-9_-]+)@/);
    const rawEmailPrefix = emailPrefixMatch ? emailPrefixMatch[1] : 'unknown';
    // Normalize to lowercase
    const emailPrefix = rawEmailPrefix.toLowerCase();
  ...
  private async processEmailWithLocking(job: EmailJob): Promise<void> {
    ...
    // Step 2: Extract prefix from the clean email address (from start to @)
    const emailPrefixMatch = emailAddress.match(/^([a-zA-Z0-9_-]+)@/);
    ...
    const rawEmailPrefix = emailPrefixMatch[1];
    // Normalize to lowercase
    const emailPrefix = rawEmailPrefix.toLowerCase();
