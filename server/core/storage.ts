...
export class Storage {
  ...
  async createEmailDeliveryFailure(data: any) {
    return miscStorage.createEmailDeliveryFailure(data);
  }

  async getUnresolvedEmailFailures(userId: string) {
    return miscStorage.getUnresolvedEmailFailures(userId);
  }

  async getUnresolvedEmailFailuresCount(userId: string) {
    return miscStorage.getUnresolvedEmailFailuresCount(userId);
  }

  async getAllEmailFailures(filters?: any) {
    return miscStorage.getAllEmailFailures(filters);
  }

  async getEmailFailureAnalytics(days?: number) {
    return miscStorage.getEmailFailureAnalytics(days);
  }

  async markEmailFailureAsResolved(id: number, actionTaken?: string) {
    return miscStorage.markEmailFailureAsResolved(id, actionTaken);
  }

  async incrementEmailFailureRetry(id: number) {
    return miscStorage.incrementEmailFailureRetry(id);
  }

  async markEmailFailureAsNotified(id: number) {
    return miscStorage.markEmailFailureAsNotified(id);
  }

  async getEmailFailureById(id: number) {
    return miscStorage.getEmailFailureById(id);
  }

  async deleteEmailFailure(id: number) {
    return miscStorage.deleteEmailFailure(id);
  }

  // ===== SUPPRESSED EMAIL METHODS =====
  async addSuppressedEmail(data: any) {
    return miscStorage.addSuppressedEmail(data);
  }

  async getSuppressedEmail(email: string) {
    return miscStorage.getSuppressedEmail(email);
  }

  async removeSuppressedEmail(email: string) {
    return miscStorage.removeSuppressedEmail(email);
  }

  async getAllSuppressedEmails() {
    return miscStorage.getAllSuppressedEmails();
  }
}
