import { bookingCollaborationService } from './booking-collaboration-service';
import { contractStorage } from '../storage/contract-storage';

export class ContractSigningEmailService {

  /**
   * Send contract signing confirmation email with booking collaboration access
   */
  async sendSigningConfirmation(
    ...
    try {
      // Generate booking collaboration access
      const portalAccess = await bookingCollaborationService.setupClientPortal(contract.id);

      // Update contract with collaboration information
      // Note: clientPortal* fields are historical names, they actually store booking collaboration URLs
      await contractStorage.updateContract(contract.id, {
        clientPortalUrl: portalAccess.portalUrl,        // Points to /booking/:id/collaborate
        clientPortalToken: portalAccess.portalToken,    // Token for booking collaboration
        clientPortalQrCode: portalAccess.qrCode         // QR code for collaboration URL
      });
