...
import type { Contract, Invoice, UserSettings } from '../../shared/schema.js';
...
function generateContractHTML(
  ...
  console.log('🎨 Generating external contract design...');

  // Use the exact HTML template provided by external AI
  return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  ...
  <div class="container">
    ...
    <div class="signature-section">
      <div class="signature-box">
        ...
        \${contract.signedAt && contract.clientSignature ? \`
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .invoice-number { font-size: 24px; font-weight: bold; color: #9333ea; margin-bottom: 5px; }
    .invoice-date { color: #666; }
    .parties { display: grid; grid-template-columns: 1fr 1fr; gap: 40px; margin-bottom: 40px; }
    .party h3 { color: #9333ea; margin-bottom: 10px; font-size: 16px; }
    .items-table { width: 100%; border-collapse: collapse; margin-bottom: 30px; }
    .items-table th { background: #f8f9fa; padding: 12px; text-align: left; border-bottom: 2px solid #e9ecef; }
    .items-table td { padding: 12px; border-bottom: 1px solid #e9ecef; }
    .total-section { margin-left: auto; width: 300px; }
    .total-row { display: flex; justify-content: space-between; padding: 8px 0; }
    .total-row.final { font-weight: bold; font-size: 18px; border-top: 2px solid #9333ea; padding-top: 12px; color: #9333ea; }
  ...
  <div class="container">
    <div class="header">
      ...
      <div class="invoice-info">
        ...
        <div class="invoice-date">Due: ${new Date(invoice.dueDate).toLocaleDateString('en-GB')}</div>
      </div>
    </div>
    ...
    <div class="parties">
      <div class="party">
        ...
        ${businessEmail}<br>
        ${businessPhone}<br>
        ${businessAddress.replace(/,/g, '<br>')}
      </div>
      <div class="party">
        <h3>Bill To</h3>
        <strong>${invoice.clientName}</strong><br>
        ${invoice.clientEmail || ''}<br>
        ${invoice.clientAddress?.replace(/,/g, '<br>') || ''}
      </div>
    </div>
    ...
    <table class="items-table">
      ...
      <tbody>
        <tr>
          ...
          <td style="text-align: right;">£${invoice.amount || '0.00'}</td>
        </tr>
      </tbody>
    </table>
    ...
    <div class="footer">
      ...
      <p>Generated by MusoBuddy Professional Services</p>
    </div>
  </div>
</body>
</html>`;
...
function getLogoBase64(): string | null {
  try {
    const logoPath = path.join(process.cwd(), 'assets', 'musobuddy-logo.png');
