...
export async function registerRoutes(app: Express) {
  ...
  app.post('/api/webhook/mailgun', async (req, res) => {
    try {
      console.log('📧 MAILGUN WEBHOOK: Received email forwarding request');
      ...
      // Use OpenAI to parse the email content and extract booking details
      const emailContent = strippedText || bodyText || bodyHtml || '';
      
      try {
        console.log('🤖 Parsing email with AI...');
        
        // Simple AI prompt to extract booking information
        const extractedData = await mailgunService.parseEmailContent(emailContent, sender, subject);
        
        console.log('🤖 AI extracted data:', extractedData);

        // Create booking entry
        const bookingData = {
          clientName: extractedData.clientName || sender.split('@')[0].replace(/[^\w\s]/gi, ''),
          clientEmail: sender,
          clientPhone: extractedData.clientPhone || '',
          clientAddress: extractedData.clientAddress || '',
          venue: extractedData.venue || 'TBC',
          venueAddress: extractedData.venueAddress || '',
          eventDate: extractedData.eventDate || new Date(Date.now() + 7 * 24 * 60 * 60 * 1000), // Default to 1 week from now
          eventTime: extractedData.eventTime || '19:00',
          eventEndTime: extractedData.eventEndTime || '22:00',
          fee: extractedData.fee || 0,
          equipmentRequirements: extractedData.equipmentRequirements || '',
        ...
        // Find default user (assumes first user for email bookings)
        const users = await storage.getAllUsers();
        const defaultUser = users.find(u => u.email === 'timfulker@gmail.com') || users[0];
        
        if (!defaultUser) {
          console.log('❌ No default user found for email booking');
          return res.status(500).json({ error: 'No user available for booking creation' });
        }

        console.log('📝 Creating booking for user:', defaultUser.email);
        const booking = await storage.createBooking(defaultUser.id, bookingData);
        
        console.log('✅ MAILGUN WEBHOOK SUCCESS: Created booking #' + booking.id);
        console.log('📧 Email from:', sender);
        console.log('📧 Subject:', subject);
        console.log('📧 Client:', bookingData.clientName);
        console.log('📧 Venue:', bookingData.venue);
        console.log('📧 Date:', bookingData.eventDate);

        res.status(200).json({ 
          success: true, 
          bookingId: booking.id,
          message: 'Email processed and booking created successfully'
        });

      } catch (aiError: any) {
        console.error('❌ AI parsing failed:', aiError);
        
        // Fallback: create basic booking with minimal data
        const fallbackBookingData = {
          clientName: sender.split('@')[0].replace(/[^\w\s]/gi, ''),
          clientEmail: sender,
          clientPhone: '',
          clientAddress: '',
          venue: 'TBC - From Email',
          venueAddress: '',
          ...
          specialRequirements: `Original email subject: ${subject}\n\nOriginal message:\n${emailContent.substring(0, 500)}...`,
          status: 'new',
          source: 'email',
          originalEmail: {
            subject,
        ...
        if (defaultUser) {
          const booking = await storage.createBooking(defaultUser.id, fallbackBookingData);
          console.log('✅ MAILGUN WEBHOOK FALLBACK: Created basic booking #' + booking.id);
          
          res.status(200).json({ 
            success: true, 
            bookingId: booking.id,
            message: 'Email processed with fallback data - manual review needed'
          });
        } else {
          res.status(500).json({ error: 'No user available and AI parsing failed' });
        }
      }
