...
export class SupabaseBookingStorage {
  ...
  async createBooking(bookingData: any) {
    ...
    // Add payment terms fields (single source of truth for contracts/invoices)
    if (bookingData.paymentTerms !== undefined && bookingData.paymentTerms !== null) {
      supabaseData.payment_terms = bookingData.paymentTerms;
    }
    if (bookingData.dueDate !== undefined && bookingData.dueDate !== null) {
      supabaseData.due_date = bookingData.dueDate;
    }
    if (bookingData.paymentTermsCustomized !== undefined && bookingData.paymentTermsCustomized !== null) {
      supabaseData.payment_terms_customized = bookingData.paymentTermsCustomized;
    }
    ...
    console.log('📝 Creating booking in Supabase:', {
      ...
      paymentTerms: supabaseData.payment_terms || 'None'
    });
  ...
  async getBooking(id: number, userId?: string) {
    ...
    console.log(`🎭 [SUPABASE-RAW] Booking ${id} raw data:`, {
      ...
      payment_terms_customized: data.payment_terms_customized
    });
    ...
    console.log(`🎭 [SUPABASE-MAPPED] Booking ${id} after mapping:`, {
      ...
      paymentTermsCustomized: mapped.paymentTermsCustomized
    });
  ...
  async updateBooking(id: number, updates: any, userId: string) {
    ...
    // Debug: Log payment terms before processing
    if (updates.paymentTerms || updates.dueDate || updates.paymentTermsCustomized) {
      console.log('💰 [SUPABASE] Payment terms in updates:', {
        paymentTerms: updates.paymentTerms,
        dueDate: updates.dueDate,
        paymentTermsCustomized: updates.paymentTermsCustomized
      });
    }
    ...
    Object.keys(updates).forEach(key => {
      ...
      const fieldMappings: { [key: string]: string } = {
        ...
        'paymentTerms': 'payment_terms',  // Payment terms fields (single source of truth)
        'dueDate': 'due_date',
        'paymentTermsCustomized': 'payment_terms_customized',
        'collaborationToken': 'collaboration_token',  // Collaboration token fields
        'collaborationTokenGeneratedAt': 'collaboration_token_generated_at',
      };
  ...
  private mapToCamelCase(booking: any) {
    return {
      ...
      paymentTermsCustomized: booking.payment_terms_customized,

      // Collaboration token fields
      collaborationToken: booking.collaboration_token,
      collaborationTokenGeneratedAt: booking.collaboration_token_generated_at,

      // Debug payment terms
      ...(booking.payment_terms || booking.due_date || booking.payment_terms_customized ? {
        _debug_payment_terms: {
          raw_payment_terms: booking.payment_terms,
          raw_due_date: booking.due_date,
          raw_customized: booking.payment_terms_customized
        }
      } : {}),
