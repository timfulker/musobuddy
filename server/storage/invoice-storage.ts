...
export class InvoiceStorage {
  ...
  async getInvoice(id: number) {
    ...
    // Fetch payment terms from linked booking if bookingId exists
    if (invoice?.bookingId) {
      try {
        const { storage } = await import("../core/storage");
        const booking = await storage.getBooking(invoice.bookingId);
        if (booking) {
          // Attach payment terms from booking to invoice
          (invoice as any).paymentTerms = booking.paymentTerms;
          (invoice as any).paymentTermsCustomized = booking.paymentTermsCustomized;
        }
      } catch (err) {
        console.warn(`⚠️ Failed to fetch payment terms from booking ${invoice.bookingId}:`, err);
      }
    }
