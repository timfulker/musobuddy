...
function generateTermsSection(userSettings: UserSettings | null, contract?: any): string {
  ...
  // Add payment terms from contract (Single Source of Truth) or fallback to settings
  const paymentTermsToUse = contract?.paymentTerms || userSettings?.contractClauses?.paymentTerms || userSettings?.invoicePaymentTerms;
  console.log(`💰 [CONTRACT-PDF] Payment terms decision for contract ${contract?.id}:`, {
    contractPaymentTerms: contract?.paymentTerms,
    settingsContractPaymentTerms: userSettings?.contractClauses?.paymentTerms,
    settingsInvoicePaymentTerms: userSettings?.invoicePaymentTerms,
  ...
  try {
    ...
    try {
      ...
      if (userSettings?.contractClauses) {
        ...
        // Use payment terms from contract (booking - single source of truth) or fallback to settings
        const paymentTermsForAI = contract?.paymentTerms || userSettings.contractClauses.paymentTerms;
        if (paymentTermsForAI) {
          const paymentTermsText = getPaymentTermsText(paymentTermsForAI);
    ...
    console.error('❌ CRITICAL: PDF generation failed for contract:', contract.contractNumber);
