...
export class AIOrchestrator {
  ...
  async runTask(taskId: string, request: AIRequest, config: TaskConfig, userId?: string): Promise<OrchestrationResult> {
    ...
    for (let i = 0; i < Math.min(availableModels.length, maxAttempts); i++) {
      ...
      try {
        ...
        const response = await this.callModel(model, request, requestId, userId);
  ...
  private async callModel(model: AIModel, request: AIRequest, requestId: string, userId?: string): Promise<AIResponse> {
    ...
    if (config.provider === 'openai') {
      ...
      if (userId) {
        await trackApiUsage({
          ...
          inputTokens,
          outputTokens,
          metadata: { requestId, taskType: 'chat_completion' }
        });
      }
      ...
      if (userId) {
        await trackApiUsage({
          ...
          inputTokens,
          outputTokens,
          metadata: { requestId, taskType: 'message_completion' }
        });
      }
