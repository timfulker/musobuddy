...
export const users = pgTable("users", {
  ...
  // Widget fields
  // IMPORTANT: Email prefixes are normalized to lowercase for case-insensitive matching
...
export const contracts = pgTable("contracts", {
  ...
  // Client portal access (DEPRECATED - now points to booking collaboration)
  // These fields store URLs to /booking/:bookingId/collaborate (the active system)
  // Kept for historical data; legacy systems removed 2025-10-06
  clientPortalUrl: text("client_portal_url"), // URL for client collaboration (points to booking-collaborate)
  clientPortalToken: text("client_portal_token"), // Token used for booking collaboration access
  clientPortalQrCode: text("client_portal_qr_code"), // QR code for client collaboration URL
...
// Email delivery failures table - tracks failed emails for user notification
export const emailDeliveryFailures = pgTable("email_delivery_failures", {
  id: serial("id").primaryKey(),
  userId: varchar("user_id").notNull().references(() => users.id, { onDelete: 'cascade' }),
  bookingId: integer("booking_id").references(() => bookings.id, { onDelete: 'cascade' }), // Optional - may not be linked to booking
  communicationId: integer("communication_id").references(() => clientCommunications.id), // Link to communication record
  recipientEmail: varchar("recipient_email").notNull(),
  recipientName: varchar("recipient_name"),
  emailType: varchar("email_type").notNull(), // 'contract', 'invoice', 'response', 'follow_up', 'other'
  subject: text("subject"),
  failureType: varchar("failure_type").notNull(), // 'hard_bounce', 'soft_bounce', 'spam_complaint', 'blocked', 'invalid'
  failureReason: text("failure_reason"), // Detailed reason from email provider
  provider: varchar("provider"), // 'sendgrid', 'mailgun'
  providerMessageId: varchar("provider_message_id"), // Message ID from email provider
  retryCount: integer("retry_count").default(0),
  lastRetryAt: timestamp("last_retry_at"),
  resolved: boolean("resolved").default(false), // User has acknowledged/handled this
  resolvedAt: timestamp("resolved_at"),
  actionTaken: text("action_taken"), // Notes about what user did to resolve
  priority: varchar("priority").default("medium"), // 'low', 'medium', 'high', 'critical'
  notifiedUser: boolean("notified_user").default(false), // Whether user has been notified
  notifiedAt: timestamp("notified_at"),
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at").defaultNow(),
}, (table) => [
  index("idx_email_failures_user").on(table.userId),
  index("idx_email_failures_booking").on(table.bookingId),
  index("idx_email_failures_resolved").on(table.resolved),
  index("idx_email_failures_type").on(table.emailType),
  index("idx_email_failures_created").on(table.createdAt),
]);

// Suppressed emails table - prevents sending to known bad email addresses
export const suppressedEmails = pgTable("suppressed_emails", {
  id: serial("id").primaryKey(),
  email: varchar("email", { length: 255 }).unique().notNull(),
  reason: varchar("reason", { length: 50 }).notNull(), // 'hard_bounce', 'soft_bounce_limit', 'spam_complaint'
  firstBounceDate: timestamp("first_bounce_date").notNull().defaultNow(),
  bounceCount: integer("bounce_count").notNull().default(1),
  provider: varchar("provider", { length: 20 }), // 'sendgrid', 'mailgun'
  notes: text("notes"),
  ...
  index("idx_suppressed_emails_created").on(table.createdAt),
]);
...
export const apiUsageTracking = pgTable("api_usage_tracking", {
  id: serial("id").primaryKey(),
  userId: varchar("user_id").notNull().references(() => users.id, { onDelete: 'cascade' }),
  service: varchar("service").notNull(), // 'openai', 'anthropic', 'google', etc.
...
export const apiUsageStats = pgTable("api_usage_stats", {
  ...
  serviceBreakdown: jsonb("service_breakdown"), // { openai: { requests: 100, cost: 5.00 }, ... }
  updatedAt: timestamp("updated_at").defaultNow(),
});

// Insert schemas for API usage
export const insertApiUsageTrackingSchema = createInsertSchema(apiUsageTracking);
export const insertApiUsageStatsSchema = createInsertSchema(apiUsageStats);
...
// Inbound Email Log - tracks all emails received via SendGrid webhooks
export const inboundEmailLog = pgTable("inbound_email_log", {
  id: serial("id").primaryKey(),
  webhookType: varchar("webhook_type").notNull(), // 'enquiries' or 'replies'
  ...
  userId: varchar("user_id").references(() => users.id, { onDelete: 'set null' }), // Extracted from email address
  bookingId: integer("booking_id"), // Extracted from email address
  attachmentCount: integer("attachment_count").default(0),
  attachmentNames: jsonb("attachment_names"), // Array of attachment filenames
  rawHeaders: jsonb("raw_headers"),
  spamScore: decimal("spam_score", { precision: 5, scale: 2 }),
  processingStatus: varchar("processing_status").default('received'), // 'received', 'processed', 'failed'
  errorMessage: text("error_message"),
  receivedAt: timestamp("received_at").defaultNow(),
  processedAt: timestamp("processed_at"),
});

export const insertInboundEmailLogSchema = createInsertSchema(inboundEmailLog);
