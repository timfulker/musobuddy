...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post('/api/bookings', isAuthenticated, async (req: any, res) => {
    try {
      ...
      if (booking.eventDate && booking.venue && booking.clientName) {
        ...
        } catch (calendarBookingError) {
          console.error('Failed to auto-create calendar booking:', calendarBookingError);
  ...
  app.delete('/api/bookings/:id', isAuthenticated, async (req: any, res) => {
    try {
      ...
      try {
        ...
        const relatedCalendarBookings = calendarBookings.filter(calendarBooking => 
          calendarBooking.notes && calendarBooking.notes.includes(`Auto-created from booking #${id}`)
        );
        
        for (const calendarBooking of relatedCalendarBookings) {
        ...
        console.error('Failed to delete related calendar bookings:', calendarBookingError);
  ...
  // Email booking intake route (manual testing)
  app.post('/api/bookings/email-intake', async (req, res) => {
    try {
      const { from, subject, body, receivedAt } = req.body;
      
      // Extract key information from email
      const { parseEmailBooking } = await import('./email-parser');
      const bookingData = await parseEmailBooking(from, subject, body);
      
      // Create booking with extracted data
      ...
      res.json({ message: "Email booking processed successfully", booking });
    } catch (error) {
      console.error("Error processing email booking:", error);
      res.status(500).json({ message: "Failed to process email booking" });
    }
  });
  ...
  // Calendar booking routes (renamed from original "booking" routes to avoid conflicts)
  app.get('/api/calendar/bookings', isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      const calendarBookings = await storage.getCalendarBookings(userId);
  ...
  app.get('/api/calendar/bookings/upcoming', isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      const calendarBookings = await storage.getUpcomingCalendarBookings(userId);
      res.json(calendarBookings);
    } catch (error) {
      console.error("Error fetching upcoming calendar bookings:", error);
      res.status(500).json({ message: "Failed to fetch upcoming calendar bookings" });
    }
  });

  app.post('/api/calendar/bookings', isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      const data = { ...req.body, userId };
      ...
      const calendarBookingData = insertBookingSchema.parse(data);
      const calendarBooking = await storage.createCalendarBooking(calendarBookingData);
      res.status(201).json(calendarBooking);
    } catch (error) {
      console.error("Error creating calendar booking:", error);
      res.status(500).json({ message: "Failed to create calendar booking" });
    }
  });

  // Update calendar booking (supports both status updates and detailed information updates)
  app.patch('/api/calendar/bookings/:id', isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      const calendarBookingId = parseInt(req.params.id);
      const updateData = { ...req.body };
      ...
      // Update the calendar booking with all provided data
      const updatedCalendarBooking = await storage.updateCalendarBooking(calendarBookingId, updateData, userId);
      
      if (!updatedCalendarBooking) {
        return res.status(404).json({ error: 'Calendar booking not found' });
      }
      
      res.json(updatedCalendarBooking);
    } catch (error) {
      console.error("Error updating calendar booking:", error);
      res.status(500).json({ message: "Failed to update calendar booking" });
    }
  });
  ...
  // Send compliance documents to calendar booking client
  app.post('/api/calendar/bookings/:bookingId/send-compliance', isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      const calendarBookingId = parseInt(req.params.bookingId);
      ...
      // Get calendar booking details
      const calendarBooking = await storage.getCalendarBooking(calendarBookingId, userId);
      if (!calendarBooking) {
        return res.status(404).json({ message: "Calendar booking not found" });
      }
      ...
      // Format event date
      const eventDate = new Date(calendarBooking.eventDate).toLocaleDateString('en-GB', {
        weekday: 'long',
        year: 'numeric',
        month: 'long',
        day: 'numeric'
      ...
      const emailText = `
Compliance Documents for ${calendarBooking.title}

Event Details:
- Event: ${calendarBooking.title}
- Date: ${eventDate}
- Venue: ${calendarBooking.venue}
...
Powered by MusoBuddy – less admin, more music
      ...
      const emailSuccess = await sendEmail({
        ...
        subject: `Compliance Documents - ${calendarBooking.title}`,
        text: emailText,
        html: emailHtml,
        replyTo: userSettings?.businessEmail || undefined,
  ...
  app.post('/api/calendar/import', isAuthenticated, upload.single('icsFile'), async (req: any, res) => {
    try {
      ...
      // Convert events to MusoBuddy calendar bookings
      ...
      res.json({
        ...
        message: `Successfully imported ${conversionResult.created} calendar bookings from calendar file`
      });
    } catch (error) {
...
Hotel Lobby Entertainment`;
  ...
  return httpServer;
}style="margin-top: 0; color: #333;">Contract Details</h3>
