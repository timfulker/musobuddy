...
export default function ActionableEnquiries() {
  ...
  const [selectedConflictEnquiry, setSelectedConflictEnquiry] = useState<Enquiry | null>(null);
  ...
  const { data: upcomingBookings = [] } = useQuery<Enquiry[]>({
  ...
  const { data: conflicts = [] } = useQuery<any[]>({
    ...
    staleTime: 30000,
    gcTime: 5 * 60 * 1000, // Fixed: changed from cacheTime to gcTime
  });
  ...
  const detectConflicts = (enquiry: Enquiry) => {
    ...
    upcomingBookings.forEach((booking: Enquiry, index: number) => {
    ...
    enquiries.forEach((otherEnquiry: Enquiry, index: number) => {
  ...
  const renderEnquiryCard = (enquiry: Enquiry, showUrgent = false) => {
    ...
    if (detectedConflicts.length > 0) {
      ...
      severity = hasConfirmedConflicts ? 'critical' : 'warning';
    }

    // Get the same card styling as bookings page
    const cardStyling = hasConflicts ? (
      severity === 'critical' ? 'border-l-4 border-l-red-500 bg-red-50 shadow-md' :
  ...
  return (
    <>
      ...
      {/* TODO: Add ConflictResolutionDialog when component is available */}
      {/* 
