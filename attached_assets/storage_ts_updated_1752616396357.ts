import {
  ...
  type User,
  type UpsertUser,
  type Enquiry as Booking,
  type InsertEnquiry as InsertBooking,
  type Contract,
  type InsertContract,
  type Invoice,
  type InsertInvoice,
  type Booking as ActualBooking,
  type InsertBooking as InsertActualBooking,
...
export interface IStorage {
  ...
  // Actual Booking operations
  getActualBookings(userId: string): Promise<ActualBooking[]>;
  getUpcomingActualBookings(userId: string): Promise<ActualBooking[]>;
  getActualBooking(id: number, userId: string): Promise<ActualBooking | undefined>;
  createActualBooking(booking: InsertActualBooking): Promise<ActualBooking>;
  updateActualBooking(id: number, booking: Partial<InsertActualBooking>, userId: string): Promise<ActualBooking | undefined>;
  deleteActualBooking(id: number, userId: string): Promise<boolean>;
...
export class DatabaseStorage implements IStorage {
  ...
  // Booking operations (using enquiries table but booking terminology)
  ...
  async createBooking(booking: InsertBooking): Promise<Booking> {
    console.log('🔍 Storage createBooking called');
    console.log('🔍 Booking title:', booking.title);
    console.log('🔍 Client name:', booking.clientName);
    console.log('🔍 Event date type:', typeof booking.eventDate);
    console.log('🔍 Event date instanceof Date:', booking.eventDate instanceof Date);
    ...
    console.log('🔍 Processed booking - eventDate:', processedBooking.eventDate);
  ...
  // Actual Booking operations (separate table for calendar events)
  async getActualBookings(userId: string): Promise<ActualBooking[]> {
    return await db
      .select()
      .from(actualBookings)
      .where(eq(actualBookings.userId, userId))
      .orderBy(desc(actualBookings.eventDate));
  }

  async getUpcomingActualBookings(userId: string): Promise<ActualBooking[]> {
    const now = new Date();
    return await db
      .select()
      .from(actualBookings)
      .where(and(eq(actualBookings.userId, userId), gte(actualBookings.eventDate, now)))
      .orderBy(actualBookings.eventDate)
      .limit(10);
  }

  async getActualBooking(id: number, userId: string): Promise<ActualBooking | undefined> {
    const [booking] = await db
      .select()
      .from(actualBookings)
      .where(and(eq(actualBookings.id, id), eq(actualBookings.userId, userId)));
    return booking;
  }

  async createActualBooking(booking: InsertActualBooking): Promise<ActualBooking> {
    const [newBooking] = await db
      .insert(actualBookings)
      .values(booking)
      .returning();
    return newBooking;
  }

  async updateActualBooking(id: number, booking: Partial<InsertActualBooking>, userId: string): Promise<ActualBooking | undefined> {
    const [updatedBooking] = await db
      .update(actualBookings)
      .set({ ...booking, updatedAt: new Date() })
      .where(and(eq(actualBookings.id, id), eq(actualBookings.userId, userId)))
      .returning();
    return updatedBooking;
  ...
  async deleteActualBooking(id: number, userId: string): Promise<boolean> {
    const result = await db.delete(actualBookings)
      .where(and(eq(actualBookings.id, id), eq(actualBookings.userId, userId)));
    return result.rowCount > 0;
  }
  ...
  async getDashboardStats(userId: string): Promise<{
    ...
    const activeBookingsCount = await db
      ...
      .where(
        and(
          eq(actualBookings.userId, userId),
          eq(actualBookings.status, "confirmed"),
          gte(actualBookings.eventDate, now)
        )
      );
    ...
    const conversionRate = totalBookings.length > 0 
      ? (confirmedBookingsCount.length / totalBookings.length) * 100 
      : 0;
