...
async function parseEmailWithAI(emailBody: string, subject: string): Promise<{
  ...
  try {
    ...
    const prompt = `Parse this email booking for a musician/performer and extract structured information. Return JSON only.
...
app.post('/api/webhook/mailgun', express.urlencoded({ extended: true }), async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Check for conflicts BEFORE creating booking (better timing)
    ...
    if (eventDate) {
      try {
        ...
        // Create temporary booking object for conflict checking
        const tempBooking = {
          ...
          status: 'new' as const,
          notes: bodyField || 'Email booking with no body content',
        ...
        const { conflicts } = await conflictService.checkBookingConflicts(tempBooking, '43963086');
    ...
    // Create booking with AI-parsed data AND conflict flags
    const booking = {
      userId: '43963086',
      title: subjectField || `Email from ${clientName}`,
      clientName,
    ...
    console.log(`📧 [${requestId}] Creating booking for: ${clientName} (${clientEmail})`);
    console.log(`📧 [${requestId}] Booking data:`, JSON.stringify(booking, null, 2));
    
    const newBooking = await storage.createBooking(booking);
    console.log(`✅ [${requestId}] Created booking #${newBooking.id}${hasConflicts ? ` (${conflictCount} conflicts detected)` : ''}`);
