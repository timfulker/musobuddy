...
export function NotificationBadge({ count, maxDisplay = 99 }: NotificationBadgeProps) {
  ...
  return (
    <span className="absolute top-3 -right-1 min-w-[18px] h-[18px] bg-red-500 text-white text-xs font-bold rounded-full flex items-center justify-center px-1 animate-pulse">
      {displayCount}
    </span>
  );
}