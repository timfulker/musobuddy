...
export default function Sidebar({ isOpen, onClose }: SidebarProps) {
  ...
  const { tooltipsEnabled, toggleTooltips } = useHelpTooltips();

  // Fetch email failures count
  const { data: emailFailuresData } = useQuery({
    queryKey: ['emailFailuresCount'],
    queryFn: async () => {
      const res = await fetch('/api/email-failures/count', {
        credentials: 'include'
      });
      if (!res.ok) throw new Error('Failed to fetch email failures count');
      return res.json();
    },
    refetchInterval: 60000, // Refetch every minute
  });

  const emailFailuresCount = emailFailuresData?.count || 0;
      ...
      >
        ...
        <nav className="flex-1 overflow-y-auto p-4 space-y-1" style={{ paddingBottom: '160px', maxHeight: 'calc(100vh - 180px)' }}>
          ...
          <Link
            ...
            onClick={() => window.innerWidth < 768 && onClose()}
            className={cn(
              getNavLinkClass("/email-failures"),
              isActive("/email-failures") ? 'bg-primary text-primary-foreground' : '',
              'relative'
            )}
          >
            <Mail className="w-5 h-5" />
            <span>Email Logs</span>
            {emailFailuresCount > 0 && <NotificationBadge count={emailFailuresCount} />}
          </Link>
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 border-t border-gray-300 dark:border-slate-700 bg-gray-50 dark:bg-slate-900 z-10">
          ...
          <div className="flex items-center justify-between mb-3 px-2 py-2 rounded-lg bg-white dark:bg-slate-800">
            <div className="flex items-center space-x-2">
              <HelpCircle className="w-4 h-4 text-slate-600" />
              <span className="text-sm font-medium text-slate-700 dark:text-slate-300">Help Tips</span>
            </div>
            <Switch
