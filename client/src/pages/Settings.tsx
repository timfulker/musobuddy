...
export default function Settings() {
  ...
  const generateWidgetUrl = async () => {
    ...
    try {
      const data = await apiRequest('/api/generate-widget-token', {
        method: 'POST',
      });
      setWidgetUrl(data.url);
      
      // Use the QR code that was already generated by the backend
      if (data.qrCode) {
        setQrCodeUrl(data.qrCode);
        console.log('✅ QR code received from backend');
      } else {
        console.warn('⚠️ No QR code in response from backend');
      }
