...
export default function FeedbackPage() {
  ...
  const handleAttachmentClick = async (url: string, e: React.MouseEvent) => {
    e.preventDefault();
    ...
    try {
      ...
      toast({
        title: "Error",
        description: error instanceof Error ? error.message : "Failed to open attachment",
        variant: "destructive",
      });
    }
  };
                        ...
                        {item.attachments.map((url, idx) => {
                          ...
                          return (
                            <a
                              key={idx}
                              href={url}
                              onClick={(e) => handleAttachmentClick(url, e)}
                              className="flex items-center space-x-1 px-2 py-1 bg-gray-100 dark:bg-gray-800 rounded hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors cursor-pointer"
                            >
