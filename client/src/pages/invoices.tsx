...
export default function Invoices() {
  ...
  // BUT: Don't recalculate if invoice was created from a booking (bookingId exists)
  // because the booking's due date is the single source of truth
  const performanceDate = form.watch("performanceDate");
  const bookingId = form.watch("bookingId");

  useEffect(() => {
    // Skip auto-calculation if this invoice is linked to a booking
    // The booking's due date should be used instead
    if (bookingId) {
      console.log('📅 [DUE DATE UPDATE] Skipping auto-calculation - invoice linked to booking', bookingId);
      return;
    }
  ...
  }, [performanceDate, userSettings, form, bookingId]);
        ...
        apiRequest(`/api/bookings/${bookingId}`)
          ...
          .then(booking => {
            if (booking) {
              ...
              if (booking.dueDate) {
                // Booking has due date - use it directly (avoid timezone conversion)
                dueDateStr = typeof booking.dueDate === 'string'
                  ? booking.dueDate.split('T')[0]  // Already a string, just extract date part
                  : new Date(booking.dueDate).toISOString().split('T')[0];
  ...
  const onSubmit = (data: z.infer<typeof invoiceFormSchema>) => {
    ...
    if (data.clientEmail && typeof data.clientEmail === 'string' && data.clientEmail.trim() && !data.clientEmail.includes('@')) {
    ...
    if (!data.clientEmail || (typeof data.clientEmail === 'string' && !data.clientEmail.trim())) {
      toast({
        title: "Note",
    ...
    const finalData = {
      ...
      clientName: safeTrim(data.clientName),
      clientEmail: safeTrim(data.clientEmail),
      ccEmail: safeTrim(data.ccEmail),
      clientAddress: safeTrim(data.clientAddress),
      ...
      gigType: safeTrim(data.gigType), // Include gig type
      invoiceType: data.invoiceType || "performance", // Include invoice type
      description: safeTrim(data.description), // Include description for ad-hoc invoices
    };
            ...
            <div className="grid grid-cols-12 gap-4 items-center">
              ...
              <div className="col-span-4">
                <div className="flex items-center justify-end gap-2 flex-wrap">
                  ...
                  {/* View Booking button - if invoice is linked to a booking */}
                  {invoice.bookingId && (
                    <Button
                      size="sm"
                      variant="outline"
                      className="text-xs whitespace-nowrap text-blue-600 hover:text-blue-700 min-w-[100px]"
                      onClick={() => setLocation(`/bookings?id=${invoice.bookingId}`)}
                    >
