  ...
  useEffect(() => {
    ...
    if (editingBooking && isEditMode) {
      ...
      form.reset({
        ...
        dueDate: formatDate(editingBooking.dueDate),
        paymentTermsCustomized: editingBooking.paymentTermsCustomized || false,
      });
  ...
  const createBookingMutation = useMutation({
    mutationFn: async (data: FullBookingFormData) => {
      const bookingData = {
        ...
        dueDate: data.dueDate instanceof Date ? data.dueDate : (data.dueDate ? new Date(data.dueDate) : null),
        paymentTermsCustomized: data.paymentTermsCustomized || false,
        status: "new" as const,
      };
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input
                            placeholder="e.g. 2 hours, 2-hour saxophone, 2 x 45 min sets"
                            {...field}
                          />
                        </FormControl>
                        <FormDescription>
                          Enter the performance duration or service selected
                        </FormDescription>
                        <FormMessage />
                      </FormItem>
                    )}
