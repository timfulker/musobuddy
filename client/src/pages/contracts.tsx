...
export default function Contracts() {
  ...
  // Clear amendment status mutation
  const clearAmendmentMutation = useMutation({
    mutationFn: async (contractId: number) => {
      return apiRequest(`/api/contracts/${contractId}/clear-amendment`, {
        method: "POST",
      });
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/contracts"] });
      toast({
        title: "Amendment Status Cleared",
        description: "The amendment pending status has been removed.",
      });
    },
    onError: (error) => {
      console.error('Clear amendment error:', error);
      toast({
        title: "Error",
        description: error.message || "Failed to clear amendment status",
        variant: "destructive",
      });
    },
  });
  ...
  */
  ...
  const handleViewSignedContract = async (contract: Contract) => {
    ...
    try {
      // CRITICAL FIX: Use isolated R2 URL endpoint with proper authentication
      console.log('🔗 FIXED: Getting R2 URL from isolated endpoint...');
      const response = await apiRequest(`/api/isolated/contracts/${contract.id}/r2-url`, {
        method: 'GET',
      });
                      ...
                      <CardContent className="p-6">
                        <div className="flex flex-col lg:flex-row lg:items-center gap-4">
                          <div className="flex items-start gap-3 flex-1 min-w-0">
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center space-x-3 mb-3">
                                ...
                                {/* Show pending amendment indicator with clear option */}
                                {contract.status === "signed" && contract.supersededBy && (
                                  <div className="flex items-center gap-2">
                                    ...
                                    <Button
                                      size="sm"
                                      variant="ghost"
                                      className="h-5 text-xs text-amber-700 hover:text-amber-900 hover:bg-amber-50 px-2"
                                      onClick={(e) => {
                                        e.stopPropagation();
                                        if (confirm("Clear the amendment pending status? This will remove the badge but won't affect the signed contract.")) {
                                          clearAmendmentMutation.mutate(contract.id);
                                        }
                                      }}
                                      data-testid={`button-clear-amendment-${contract.id}`}
                                    >
                                      Clear
                                    </Button>
                                  </div>
                          ...
                          <div className="flex flex-wrap items-center gap-2 lg:flex-nowrap lg:flex-shrink-0 lg:justify-start">
                            ...
                            {/* View Booking button - if contract is linked to a booking */}
                            {contract.enquiryId && (
                              <Button
                                size="sm"
                                variant="outline"
                                className="text-xs whitespace-nowrap text-blue-600 hover:text-blue-700"
                                onClick={() => navigate(`/bookings?id=${contract.enquiryId}`)}
                              >
