...
CRITICAL REGRESSION (Aug 15, 2025): Storage webhook handling for Encore emails with attachments regressed after deployment at 02:32. Successful Encore processing at 00:53 vs failed processing at 02:36 confirms deployment broke previously working system. Issue: webhook acknowledges storage webhooks with 200 OK but doesn't fetch email content from Mailgun storage URLs. Files involved: server/index.ts lines 85-126 (storage webhook handler), server/core/email-queue-enhanced.ts (processing logic), server/ai/booking-message-parser.ts (Encore detection). External review requested due to critical nature for production launch.
