...
- **"View Full Contract PDF" Button Fixed on Signing Pages**: Resolved the 404 error when clicking the "View Full Contract PDF" button on R2-hosted contract signing pages. The button now correctly points to the new public PDF API endpoint (`/api/contracts/public/:id/pdf`) instead of broken cloud storage URLs. Added the missing public PDF route before the catch-all middleware to ensure proper route registration. Contract signing pages now provide reliable PDF access for clients reviewing contracts before signing (August 5, 2025)
