...
Encore booking management: Added "Applied" button for Encore bookings on dashboard that automatically updates booking status to "in_progress" and removes them from action-required list. This prevents dashboard clutter from applied-for jobs while maintaining booking records. Button appears alongside existing "Apply on Encore" button with green styling to indicate completion action. Extended "Applied" button functionality to booking cards on main bookings page for consistent workflow across platform.
