...
Dual Stripe configuration: System has two separate Stripe integrations - invoice payments (invoice-routes.ts) and user subscriptions (stripe-service.ts). Both require test mode configuration for development. Critical fix applied to force test mode in both systems to prevent live mode conflicts during testing.
Invoice payment workflow: Removed Stripe invoice payments to avoid 1.4% commission fees for musicians. Invoices now display bank details for direct bank transfers. Manual payment tracking available through "Mark as Paid" functionality. Future Stripe Connect integration preserved in backup for potential reactivation if market demands automated payments.
Professional email styling: Invoice emails now use modern, professional design with responsive layout, Google Fonts (Inter), gradient headers using user's theme color, organized invoice details sections, and branded styling that matches the MusoBuddy platform quality. Replaced basic HTML with full responsive email template including proper typography, mobile optimization, and single professional "Download Invoice PDF" button (removed dead "Pay Now" link to align with bank transfer payment workflow).
Authentication system critical fix: Fixed recurring authentication bug where users were getting logged into wrong accounts. Root cause identified: Stripe payment redirects were using development URLs instead of production URLs due to faulty environment detection, causing users to switch environments mid-signup. Fixed by hardcoding production URLs in Stripe checkout sessions. Also resolved 404 loop issue by registering missing authentication routes and added infinite retry loop prevention in useAuth hook.
...
Google Calendar text visibility: Fixed critical UI contrast issues where text in Calendar Integration dialog was nearly invisible due to poor color choices. Replaced `text-muted-foreground` with proper `text-gray-900/700/600` contrast levels for optimal readability across all sections.
Comprehensive onboarding wizard: Implemented multi-step setup wizard for new users covering business information, contact details with critical email prefix setup, pricing rates, service areas, and theme branding. Email prefix field is the most important component as it creates personalized booking email addresses (e.g., tim@enquiries.musobuddy.com). System includes proper validation, uniqueness checking, and prevents changes after setup. Wizard automatically appears for users with `onboardingCompleted: false` and integrates seamlessly with existing authentication flow.
